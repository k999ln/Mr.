...
# Channels where every message routes to Anicca (no @mention needed).
# Dais 2026-05-30: "I don't want to mention Anicca every time I talk to him
# on Slack. Make it so every message on #metrics goes straight to him."
ANICCA_AUTO_ROUTE_CHANNELS = {"C091G3PKHL2"}  # #metrics


@app.event("message")
def handle_message(event, say):
    """DM + auto-route channels → task file. Other channels still require @mention."""
    # Ignore bot messages, edited messages, and channel-history backfills.
    if event.get("subtype") in ("bot_message", "message_changed", "message_deleted"):
        return
    # Skip thread replies — top-level channel messages only (avoids
    # firing on Anicca's own in-thread replies and on metadata threads).
    ...
    #   1. Direct messages (channel_type=im)         — existing behavior
    #   2. Auto-route channels (e.g. #metrics)       — new Dais 2026-05-30
    # Channel @mentions still arrive via app_mention; handling them here would
    # double-fire, so we still skip non-auto-route channels.
    is_dm = channel_type == "im"
    is_auto_route = channel in ANICCA_AUTO_ROUTE_CHANNELS
    if not (is_dm or is_auto_route):
        return
    ...
    # message matches its regex, route there and skip task creation.
    if text and _check_awaiting(text):
        return
    prefix = "Slack DM" if is_dm else "Slack metrics"
