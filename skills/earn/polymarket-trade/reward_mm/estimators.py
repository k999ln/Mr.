...
Near-verbatim port of poly-maker's src/polymaker/strategy/estimators.py
(MIT, warproxxx/poly-maker). All are time-decayed (half-life in seconds) so
a burst of ticks and a quiet minute are weighted by elapsed wall-clock, not
by sample count. Pure state machines: feed them observations with
timestamps, read scalar summaries. No I/O, no network, no judgment about
WHICH market to trade — that's gamma_scan.py's job.
"""
...
class Ewma:
    ...
    On each update the prior weight decays by 0.5 ** (dt / halflife); a
    fresh observation gets the remaining weight. The first observation
    seeds the mean.
    """
...
class FlowEstimator:
    ...
    aggressor: "BUY" or "SELL" (string, not an enum — this port has no
    exchange-connection layer yet, so there is no live trade tape feeding
    this in paper mode; kept here so a future WS integration can call it
    directly without touching the pure logic).
    """
    ...
    def update(self, aggressor: str, size: float, ts: float) -> None:
        signed = size if aggressor == "BUY" else -size
...
class _PendingMarkout:
    fv_at_fill: float
    side: str  # "BUY" | "SELL", our side of the fill
    due_ts: float
...
class MarkoutTracker:
    ...
    A *positive* markout means the trade was good (price moved in our
    favor); negative means we got picked off. Toxicity is the magnitude of
    recent adverse (negative) markout, which the quoter turns into extra
    spread / less size.
    """
    ...
    def record_fill(self, side: str, fv_at_fill: float, ts: float) -> None:
    ...
    def evaluate(self, fv_now: float, ts: float) -> None:
        ...
        for p in self._pending:
            if ts >= p.due_ts:
                ...
                signed = move if p.side == "BUY" else -move
...
class MarketEstimators:
    ...
    @staticmethod
    def new(short_halflife_s: float, long_halflife_s: float, flow_halflife_s: float) -> "MarketEstimators":
        return MarketEstimators(
            vol=VolEstimator(short_halflife_s, long_halflife_s),
