...
from call_context_store import valid_context_id
...
class DialoutRequest(BaseModel):
    ...
    to_number: str = Field(pattern=r"^\+[1-9]\d{7,14}$", max_length=16)
    from_number: str = Field(pattern=r"^\+[1-9]\d{7,14}$", max_length=16)
    mode: Literal["wakeup", "lateness"] = "wakeup"
    ctx: str = Field(default="", max_length=8000)
    name: str = Field(default="friend", max_length=120)
...
class TwimlRequest(BaseModel):
    ...
    Attributes:
        context_id (str): Opaque single-use reference to local call context.
    """
...
async def make_twilio_call(
    ...
    if not valid_context_id(context_id):
        raise ValueError("Invalid local call context id")

    # The URL contains only a random, single-use lookup id. Calendar text,
    # location context, and the owner's name never enter Twilio's callback URL.
    qs = urlencode({"context_id": context_id})
...
async def parse_twiml_request(request: Request) -> TwimlRequest:
    ...
    context_id = qp.get("context_id", "")
    if not valid_context_id(context_id):
        raise HTTPException(status_code=400, detail="invalid call context id")
    return TwimlRequest(
        to_number=to_number,
...
def generate_twiml(twiml_request: TwimlRequest) -> str:
    ...
    endpoint. Only an opaque local context id is passed as a stream parameter.
    ...
    stream.parameter(name="context_id", value=twiml_request.context_id)
