-- Allow a visitor who chose one to three tools on the website to continue directly in Telegram.
-- The free tier still caps the selection at three; this only removes the website's exact-three requirement.
...
BEGIN
  ...
  IF p_uid IS NULL OR NOT EXISTS (SELECT 1 FROM public.lm_users WHERE uid = p_uid)
     OR normalized_keys IS NULL OR cardinality(normalized_keys) NOT BETWEEN 1 AND 3
