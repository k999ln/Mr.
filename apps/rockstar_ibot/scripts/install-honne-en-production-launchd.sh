...
TEMPLATE="$APP_DIR/launchd/ai.anicca.rockstar_ibot-honne-en.plist.template"
TARGET="${HOME}/Library/LaunchAgents/ai.anicca.rockstar_ibot-honne-en.plist"
DOMAIN="gui/$(id -u)"
LABEL="ai.anicca.rockstar_ibot-honne-en"
TEMP="$(mktemp "${TMPDIR:-/tmp}/rockstar_ibot-honne-en.plist.XXXXXX")"
trap 'rm -f "$TEMP"' EXIT
