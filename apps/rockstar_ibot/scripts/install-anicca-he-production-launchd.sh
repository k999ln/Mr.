#!/usr/bin/env bash
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"; APP_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"; LABEL="ai.anicca.rockstar_ibot-anicca-he"; DOMAIN="gui/$(id -u)"
TEMPLATE="$APP_DIR/launchd/$LABEL.plist.template"; TARGET="${HOME}/Library/LaunchAgents/$LABEL.plist"; TEMP="$(mktemp "${TMPDIR:-/tmp}/rockstar_ibot-anicca-he.plist.XXXXXX")"; trap 'rm -f "$TEMP"' EXIT
