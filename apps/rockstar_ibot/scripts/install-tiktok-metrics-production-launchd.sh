...
TEMPLATE="$APP_DIR/launchd/ai.anicca.rockstar_ibot-tiktok-metrics.plist.template"; TARGET="${HOME}/Library/LaunchAgents/ai.anicca.rockstar_ibot-tiktok-metrics.plist"
DOMAIN="gui/$(id -u)"; LABEL="ai.anicca.rockstar_ibot-tiktok-metrics"; TEMP="$(mktemp "${TMPDIR:-/tmp}/rockstar_ibot-tiktok-metrics.plist.XXXXXX")"
trap 'rm -f "$TEMP"' EXIT
