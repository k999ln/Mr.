...
TEMPLATE="$APP_DIR/launchd/ai.anicca.rockstar_ibot-instagram-metrics.plist.template"; TARGET="${HOME}/Library/LaunchAgents/ai.anicca.rockstar_ibot-instagram-metrics.plist"
DOMAIN="gui/$(id -u)"; LABEL="ai.anicca.rockstar_ibot-instagram-metrics"; TEMP="$(mktemp "${TMPDIR:-/tmp}/rockstar_ibot-instagram-metrics.plist.XXXXXX")"
trap 'rm -f "$TEMP"' EXIT
