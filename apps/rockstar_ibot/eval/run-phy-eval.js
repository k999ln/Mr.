...
const { detectUnmetCare } = require("../lib/care-detector.js");

const cases = fs.readFileSync(path.join(__dirname, "phy-cases.jsonl"), "utf8").trim().split("\n").map(JSON.parse);
const failures = [];
for (const testCase of cases) {
  const actual = detectUnmetCare(testCase.input);
...
console.log(`Physical care eval: ${passed}/${cases.length} (${(passed / cases.length * 100).toFixed(1)}%) judge=deterministic`);
