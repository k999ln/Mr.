...
const { decideOpportunity } = require("../lib/opportunity-engine.js");

const NOW_MS = Date.parse("2026-07-24T09:00:00.000Z");
const cases = fs.readFileSync(path.join(__dirname, "intent-cases.jsonl"), "utf8").trim().split("\n").map(JSON.parse);
const failures = [];
for (const testCase of cases) {
  const actual = decideOpportunity(testCase.graph, testCase.candidate, NOW_MS);
...
console.log(`Intent eval: ${passed}/${cases.length} (${(passed / cases.length * 100).toFixed(1)}%) judge=deterministic`);
