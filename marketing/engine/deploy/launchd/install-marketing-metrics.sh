...
DESTINATION="$HOME/Library/LaunchAgents/ai.anicca.marketing-metrics.plist"
BOOTSTRAP=true
...
TEMPLATE="$SCRIPT_DIR/ai.anicca.marketing-metrics.plist"
...
if [[ "$BOOTSTRAP" == true ]]; then
  DOMAIN="gui/$(id -u)"
  launchctl bootout "$DOMAIN/ai.anicca.marketing-metrics" 2>/dev/null || true
  launchctl bootstrap "$DOMAIN" "$DESTINATION"
  launchctl kickstart -k "$DOMAIN/ai.anicca.marketing-metrics"
fi
